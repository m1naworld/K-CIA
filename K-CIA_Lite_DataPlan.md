...
- **D9 행정동 경계(SHP)**: 국토교통부 센서스 행정동 경계
    ...
    - 키: 행정동 코드(속성), 폴리곤(공간)
- **D12 소상공인 상가(상권)정보**: 소상공인시장진흥공단 상가정보 API
    - 링크: https://www.data.go.kr/data/15012005/openapi.do
    - 키: `bizesId` (상가업소번호)
    - 용도: 개별 점포 좌표(경도/위도) → H3 점포 수 기반 weight 계산
...
2. H3 해상도 선택(res=10 기본; 성능 문제 시 res=9 다운그레이드)
...
- **Q2 H3 해상도(res)** → **기본 res=10(확정)**
    - UX 기본 줌(골목 단위)에서 정밀한 분석을 우선
    - 성능 이슈 발생 시: res=9 / res=8로 다운그레이드 가능
