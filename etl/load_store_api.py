"""Load D2 (점포-행정동) via Seoul Open API → fact_store_area_qtr.

Usage:
    docker compose run --rm --entrypoint python etl -m etl.load_store_api
    docker compose run --rm --entrypoint python etl -m etl.load_store_api 20251
"""
...
def get_seongsu_admin_dong_mapping() -> dict[str, int]:
    """Get admin dong code to area_id mapping for Seongsu-dong."""
    result = execute_sql(
        "SELECT area_code, area_id FROM dim_area WHERE area_type = 'ADMIN_DONG'"
    )
...
def upsert_fact_store(
    ...
    for row in rows:
        dong_code = str(row.get("ADSTRD_CD", ""))
        
        area_id = area_mapping.get(dong_code)
        if area_id is None:
            continue
...
def main() -> None:
    print("=" * 60)
    print("Load D2 (점포) via API → fact_store_area_qtr")
    print("=" * 60)
    ...
    area_mapping = get_seongsu_admin_dong_mapping()
    if not area_mapping:
        print("\n[ERROR] No admin dongs in dim_area. Run load_boundaries first.")
        sys.exit(1)
    print(f"\n[Step 1] Seongsu admin dong mapping: {len(area_mapping)} dongs")
    print(f"  Codes: {list(area_mapping.keys())}")
    
    collector = SeoulAPICollector()
    service = API_SERVICES["D2_STORE"]
    ...
    for qtr in quarters:
        ...
        seongsu_rows = [r for r in rows if str(r.get("ADSTRD_CD", "")) in area_mapping]
        print(f"  Seongsu rows: {len(seongsu_rows):,}")
        
        if not seongsu_rows:
            sample_codes = list({str(r.get("ADSTRD_CD", "")) for r in rows[:20]})
            print(f"  [DEBUG] Sample dong codes from API: {sample_codes[:5]}")
            continue
    ...
    result = execute_sql("SELECT count(*) FROM fact_store_area_qtr")
    count = list(result)[0][0]
    print(f"  fact_store_area_qtr: {count:,} rows")
