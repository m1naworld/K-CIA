...
- **SNS 모듈 (M9)**: 완료 (옵션 모듈, OFF 상태에서도 Q&A 성립)
- **Phase 2 (S2+S5)**: M10(피크타임 운영전략) + M11(리스크 진단) 완료
- **다음 단계**: Phase 4 (콘텐츠 생성: 4컷/쇼츠) 계획 수립
...
* **상세 로직:**
  ...
  * **추가 레이어:** 시간대(피크 집중도) 및 리스크(폐업/경쟁/매출/유동) 모드 지원.
...
* **상세 로직:**
  ...
  * YouTube + Naver Search API를 통한 일일 단위 소셜 데이터(댓글/트렌드) 배치 수집.
  * Kakao Local API로 장소/주소를 지오코딩하여 상권 매핑(베스트 에포트).
  * SNS 모듈은 옵션이며 OFF 상태에서도 공공데이터만으로 Q&A/추천 성립.
...
| **Data Source** | Seoul Open Data, YouTube API, Naver Search API, Kakao Local API | 정량/정성 데이터 하이브리드 수집 |
